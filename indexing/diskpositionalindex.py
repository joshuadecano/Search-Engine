...
class DiskPositionalIndex(Index):
    ...
    def get_postings(self, term : str) -> Iterable[Posting]:
        ...
        for s in size:  # for each doc containing the term
            ...
            prev_docid = doc_id
            preta_path.seek(1,1) # seek to tftd (number of positions)
            position_count = struct.unpack('i', preta_path.read(1))
            prev_position = 0
            for t in position_count:
                preta_path.seek(1,1) # seek to pi, (position of term in doc_id)
                position = struct.unpack('i', preta_path.read(1))
